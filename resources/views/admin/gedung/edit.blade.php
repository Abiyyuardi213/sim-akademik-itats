...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Ubah Data Gedung</h1>
            <p class="mt-1 text-sm text-zinc-500">Perbarui informasi gedung yang sudah ada.</p>
        </div>
    ...
    <div class="max-w-2xl bg-white rounded-xl border border-zinc-200 shadow-sm p-6 sm:p-8">
        <div class="mb-6 border-b border-zinc-100 pb-4">
            <h3 class="text-lg font-semibold text-zinc-900">Form Ubah Gedung</h3>
            <p class="text-sm text-zinc-500">Edit detail informasi gedung.</p>
        </div>

        <form action="{{ route('admin.gedung.update', $gedung->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <textarea id="gedung_description" name="gedung_description" rows="4" required
                    class="flex w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-zinc-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">{{ old('gedung_description', $gedung->gedung_description) }}</textarea>
            </div>
