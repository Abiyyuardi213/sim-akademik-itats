...
@section('title', 'Ubah Peminjaman Ruangan')

@section('content')
    <!-- Page Header -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Ubah Data Peminjaman</h1>
            <p class="mt-1 text-sm text-zinc-500">Perbarui informasi jadwal peminjaman ruangan.</p>
        </div>
    ...
    <div class="max-w-4xl bg-white rounded-xl border border-zinc-200 shadow-sm p-6 sm:p-8">
        <div class="mb-6 border-b border-zinc-100 pb-4">
            <h3 class="text-lg font-semibold text-zinc-900">Form Ubah Data</h3>
            <p class="text-sm text-zinc-500">Edit detail peminjaman di bawah ini.</p>
        </div>

        <form action="{{ route('admin.peminjaman-ruangan.update', $peminjaman->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" id="tanggal_berakhir_peminjaman" name="tanggal_berakhir_peminjaman"
                                value="{{ old('tanggal_berakhir_peminjaman', $peminjaman->tanggal_berakhir_peminjaman) }}"
                                required
                                class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="time" id="waktu_peminjaman" name="waktu_peminjaman"
                                value="{{ old('waktu_peminjaman', $peminjaman->waktu_peminjaman) }}" required
                                class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="time" id="waktu_berakhir_peminjaman" name="waktu_berakhir_peminjaman"
                                value="{{ old('waktu_berakhir_peminjaman', $peminjaman->waktu_berakhir_peminjaman) }}"
                                required
                                class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">
                        </div>
                    </div>
                            ...
                            <select id="kelas_id" name="kelas_id" required
                                ...
                                @foreach ($kelass as $kelas)
                                    <option value="{{ $kelas->id }}"
                                        {{ old('kelas_id', $peminjaman->kelas_id) == $kelas->id ? 'selected' : '' }}>
                                        {{ $kelas->nama_kelas }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="space-y-2">
                ...
                <textarea id="keperluan_peminjaman" name="keperluan_peminjaman" rows="3" required
                    class="flex w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-zinc-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">{{ old('keperluan_peminjaman', $peminjaman->keperluan_peminjaman) }}</textarea>
            </div>
