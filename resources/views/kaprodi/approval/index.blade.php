...
@section('content')
                        ...
                        <tr class="hover:bg-zinc-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <p class="font-medium text-zinc-900">
                                    {{ $pengajuan->room_name ?? $pengajuan->kelas->nama_kelas }}</p>
    ...
    @if ($histories->count() > 0)
        <div class="mt-12 mb-6">
            <h2 class="text-lg font-bold text-zinc-900">Riwayat Persetujuan</h2>
            <p class="text-sm text-zinc-500">Daftar permohonan yang telah Anda proses sebelumnya.</p>
        </div>

        <div class="rounded-xl border border-zinc-200 bg-white shadow-sm overflow-hidden mb-12">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead
                        class="bg-zinc-50/50 text-zinc-500 uppercase tracking-wider font-medium border-b border-zinc-200">
                        <tr>
                            <th class="px-6 py-4 w-12 text-center">No</th>
                            <th class="px-6 py-4">Peminjam</th>
                            <th class="px-6 py-4">Ruangan</th>
                    ...
                    <tbody class="divide-y divide-zinc-100 bg-white">
                        @foreach ($histories as $index => $history)
                            <tr class="hover:bg-zinc-50/50 transition-colors group">
                                <td class="px-6 py-4 text-center font-medium text-zinc-500">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-zinc-900">{{ $history->user->username ?? '-' }}</p>
                                    <div class="flex items-center mt-1 text-xs text-zinc-500">
                                        <span
                                            class="truncate max-w-[150px]">{{ $history->prodi->nama_prodi ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="font-medium text-zinc-900">
                                        {{ $history->room_name ?? ($history->kelas->nama_kelas ?? '-') }}</p>
                                    <p class="text-xs text-zinc-500">{{ $history->kelas->gedung->nama_gedung ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4 text-zinc-600">
                                    <p class="line-clamp-2 max-w-xs" title="{{ $history->keperluan_peminjaman }}">
                                        {{ $history->keperluan_peminjaman }}
                                    </p>
                                </td>
                                <td class="px-6 py-4 text-zinc-600 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span
                                            class="font-medium">{{ \Carbon\Carbon::parse($history->tanggal_peminjaman)->translatedFormat('d M Y') }}</span>
                                        <span class="text-xs text-zinc-500 mt-0.5">
                                            {{ \Carbon\Carbon::parse($history->waktu_peminjaman)->format('H:i') }} -
                                            {{ \Carbon\Carbon::parse($history->waktu_berakhir_peminjaman)->format('H:i') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @php
                                        $statusColors = [
                                            ...
                                            'disetujui' => 'bg-emerald-50 text-emerald-700 ring-emerald-600/20',
                                            'ditolak' => 'bg-red-50 text-red-700 ring-red-600/10',
                                            'batal' => 'bg-gray-50 text-gray-700 ring-gray-600/10',
                                        ...
                                        $statusClass =
                                            ...
                                            'bg-zinc-50 text-zinc-700 ring-zinc-600/10';
                                        $statusLabel = $statusLabels[$history->status] ?? ucfirst($history->status);
                                    @endphp
                                    <span
                                        class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset {{ $statusClass }}">
                                        {{ $statusLabel }}
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('kaprodi.approval.show', $history->id) }}"
                                        class="inline-flex items-center justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-zinc-900 shadow-sm ring-1 ring-inset ring-zinc-300 hover:bg-zinc-50 transition-colors">
                                        Detail
                                    </a>
                                </td>
